...
void TsdfServer::processPointCloudMessageAndInsert(
  ...
  // Check for different pointcloud formats without modifying const data
  bool has_rgb = false;
  bool has_intensity = false;
  bool has_heat = false;
  bool has_rgb_and_heat = false;
  ...
  for (size_t d = 0; d < pointcloud_msg->fields.size(); ++d) {
    if (pointcloud_msg->fields[d].name == std::string("rgb")) {
      ...
      has_intensity = true;
    } else if (pointcloud_msg->fields[d].name == std::string("heat")) {
      has_heat = true;
    }
  }
  
  // Check if it has both rgb and heat (CLIPSeg format)
  if (has_rgb && has_heat) {
    has_rgb_and_heat = true;
  }
  ...
  // Convert differently depending on pointcloud format
  if (has_rgb_and_heat) {
    // CLIPSeg format with rgb and heat fields - manual conversion
    pcl::PointCloud<pcl::PointXYZ> pointcloud_pcl;
    pointcloud_pcl.reserve(pointcloud_msg->width * pointcloud_msg->height);
    
    // Find field offsets
    size_t x_offset = 0, y_offset = 0, z_offset = 0, heat_offset = 0;
    for (size_t d = 0; d < pointcloud_msg->fields.size(); ++d) {
      if (pointcloud_msg->fields[d].name == "x") x_offset = pointcloud_msg->fields[d].offset;
      else if (pointcloud_msg->fields[d].name == "y") y_offset = pointcloud_msg->fields[d].offset;
      else if (pointcloud_msg->fields[d].name == "z") z_offset = pointcloud_msg->fields[d].offset;
      else if (pointcloud_msg->fields[d].name == "heat") heat_offset = pointcloud_msg->fields[d].offset;
    }
    
    // Manual conversion from sensor_msgs to extract points and traversability
    for (size_t i = 0; i < pointcloud_msg->width * pointcloud_msg->height; ++i) {
      pcl::PointXYZ point;
      
      // Extract x, y, z from the pointcloud message
      size_t offset = i * pointcloud_msg->point_step;
      
      // Extract x, y, z using proper field offsets
      memcpy(&point.x, &pointcloud_msg->data[offset + x_offset], sizeof(float));
      memcpy(&point.y, &pointcloud_msg->data[offset + y_offset], sizeof(float));
      memcpy(&point.z, &pointcloud_msg->data[offset + z_offset], sizeof(float));
      
      // Extract heat value for traversability
      float heat_value;
      memcpy(&heat_value, &pointcloud_msg->data[offset + heat_offset], sizeof(float));
      
      // Check if point is finite
      if (std::isfinite(point.x) && std::isfinite(point.y) && std::isfinite(point.z)) {
        // Add point to pointcloud
        pointcloud_pcl.push_back(point);
        
        // Add traversability value
        TraversabilityValue t_ = 255 * heat_value;
        traversability_values.push_back(t_);
      }
    }
    
    // Convert points and assign colors
    convertPointcloud(pointcloud_pcl, color_map_, &points_C, &colors);
  } else if (has_rgb && !has_heat) {
    // Standard RGB format
    pcl::PointCloud<pcl::PointXYZRGB> pointcloud_pcl;
    pcl::fromROSMsg(*pointcloud_msg, pointcloud_pcl);
    convertPointcloud(pointcloud_pcl, color_map_, &points_C, &colors);
  } else if (has_intensity) {
    // Intensity format (existing functionality)
