...
MapManagerVoxblox<SDFServerType, SDFVoxelType>::MapManagerVoxblox(
  ...
  if (!node->get_parameter("occupancy_distance_voxelsize_factor",
  ...
  //     voxblox::getTsdfIntegratorConfigFromRosParam(voxblox::getTsdfMapConfigFromRosParam());
  // esdf_integrator_config_ =
  //     voxblox::getEsdfIntegratorConfigFromRosParam();

  tsdf_integrator_config_ = sdf_server_.tsdf_integrator_config;
  // esdf_integrator_config_ = sdf_server_.esdf_integrator_config; // @aakapatel
  // Fix me!.
  ...
  RCLCPP_INFO_EXPRESSION(node->get_logger(),
...
template <typename SDFServerType, typename SDFVoxelType>
float MapManagerVoxblox<SDFServerType, SDFVoxelType>::getVoxelTraversability(
    const Eigen::Vector3d& position) const {
  SDFVoxelType* voxel = sdf_layer_->getVoxelPtrByCoordinates(
      position.cast<voxblox::FloatingPoint>());

  float traversability_value = 0.0f;

  if (checkUnknownStatus(voxel)) {
    traversability_value = 0.0f;
    return traversability_value;
  }
  std::cout << "Query voxel traversability : " << voxel->traversability
            << std::endl;
  return voxel->traversability;
}
...
void MapManagerVoxblox<voxblox::EsdfServer, voxblox::EsdfVoxel>::clearIfUnknown(
  ...
  if (!voxel.observed) {
    ...
    voxel.distance =
        0;  // ROS2 migration hack. @aakapatel fix esdf integrator config.
  }
}
...
void MapManagerVoxblox<SDFServerType, SDFVoxelType>::getFreeSpacePointCloud(
    std::vector<Eigen::Vector3d> multiray_endpoints, Eigen::Vector4d state,
...
void MapManagerVoxblox<SDFServerType, SDFVoxelType>::augmentFreeFrustum() {
  ...
  RCLCPP_WARN(logger, "MapManagerVoxblox::augmentFreeFrustum: N/A");
}
...
void MapManagerVoxblox<SDFServerType, SDFVoxelType>::extractLocalMap(
    ...
    std::vector<Eigen::Vector3d>& unknown_voxels) {
  occupied_voxels.clear();
  free_voxels.clear();
  unknown_voxels.clear();
...
void MapManagerVoxblox<SDFServerType, SDFVoxelType>::getLocalPointcloud(
    ...
    pcl::PointCloud<pcl::PointXYZ>& pcl, bool include_unknown_voxels) {
