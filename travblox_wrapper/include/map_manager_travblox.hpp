...
class MapManagerVoxblox : MapManager {
 public:
  MapManagerVoxblox(rclcpp::Node::SharedPtr node);

  double getResolution() const;
  bool getStatus() const;
  float getVoxelTraversability(const Eigen::Vector3d& position) const;
  ...
  void getFreeSpacePointCloud(std::vector<Eigen::Vector3d>, Eigen::Vector4d,
